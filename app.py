...
def _read_tiket_file(f) -> pd.DataFrame:
    """Excel Tiket: auto-deteksi baris header (atasi judul/merge di baris 1)."""
    try:
        f.seek(0)
        raw = pd.read_excel(f, engine="openpyxl", header=None, dtype=str)
        if raw.empty: return pd.DataFrame()
        targets = ["action", "action date", "created", "tarif", "st bayar", "status", "bank", "channel"]
...
if go:
    # --- Cari kolom (lebih toleran) ---
    t_date = _find_col(tiket_df, ["Action", "Action date", "Created", "Created date", "Tanggal"])
    t_amt  = _find_col(tiket_df, ["tarif", "fare", "amount", "nominal", "total"])
    ...
    missing = []
    for name, col, src in [
        ("Action/Created", t_date, "Tiket Detail"),
    ...
    # --- Reindex ke tanggal 1..akhir bulan ---
