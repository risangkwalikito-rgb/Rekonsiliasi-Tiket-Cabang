...
- Normalisasi angka (format Indonesia) dan tanggal
- Agregasi per tanggal dan per kategori (ESPAY, CASH, BCA, Non-BCA)
- Hitung selisih dan total, tampilkan tabel, unduh Excel
...
def parse_rupiah_to_number(val) -> float:
    """
    Parser angka ID: terima '1.234.567', '1,234,567', '(1.000)', '-2.500'.
    Return float; gagal -> 0.0.
    """
    if pd.isna(val):
        return 0.0
    s = str(val).strip()
...
def sum_by_date(
    ...
    """
    Jumlahkan nominal per tanggal. Filter opsional via flt_col dengan include/exclude keywords.
    Cocok substring case-insensitive. include menang atas exclude.
    """
    if df.empty:
...
if go:
    ...
    if missing:
        st.error("Lengkapi unggahan dan mapping: " + ", ".join(missing))
        st.stop()
    ...
    st.caption("Sesuaikan kata kunci dan mapping jika label bank/channel berbeda.")
