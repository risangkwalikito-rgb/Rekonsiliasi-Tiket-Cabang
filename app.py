...
if go:
    ...
    # Settlement Dana: agregasi
    sd = settle_df.copy()
    raw_date_examples = sd[s_date].head(5).tolist()
    raw_amt_examples  = sd[s_amt].head(5).tolist()

    sd[s_date] = sd[s_date].apply(_to_date)
    sd = sd[~sd[s_date].isna()]
    sd[s_amt] = _to_num(sd[s_amt])

    parsed_date_examples = sd[s_date].head(5).tolist()
    parsed_amt_examples  = sd[s_amt].head(5).tolist()
    ...
    if show_debug:
        st.info(
            "DEBUG PARSING\n"
            f"- Tiket rows valid: {len(td)}\n"
            f"- Settlement rows valid: {len(sd)}\n"
            f"- Contoh Transaction Date (raw → parsed): {raw_date_examples[:3]} → {parsed_date_examples[:3]}\n"
            f"- Contoh Settlement Amount  (raw → parsed): {raw_amt_examples[:3]} → {parsed_amt_examples[:3]}"
        )
